...
module.exports = (options, plugins, hooks) => {
	...
	function shouldBeRtl(root) {
		return root.source.input.file.toLowerCase().endsWith('.rtl.scss');
	}

	return {
		postcssPlugin: 'rtlcss',
		Once(root, { result }) {
			if (!shouldBeRtl(root)) return;
				...
				state.parse(node, result, (current) => {
					...
					if (current.directive) {
						if (!current.metadata.begin && current.metadata.end) {
							...
							current.directive.expect.self &&
							current.directive.begin(node, current.metadata, context) &&
							current.metadata.end &&
				...
				if (
					!context.util.each(context.config.plugins, (plugin) => {
						return context.util.each(plugin.directives.value, (directive) => {
							...
							if (
								expr.test(
									`${node.raws.between}${hasRawValue ? node.raws.value.raw : ''}${
										node.important && node.raws.important ? node.raws.important : ''
									}`,
								)
							) {
								expr.lastIndex = 0;
